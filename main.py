...
def validate(model, val_dataset, baseline, device, epoch, writer):
    ...
    for metric, figure in figures.items():
        writer.add_figure('eval/{}'.format(metric), figure, epoch)
...
def make_eval_set(path):
    src_wav_files = filter(lambda s: s.endswith('_src.wav'), os.listdir(path))
    ...
    for src_wav_file in src_wav_files:
        ...
        mix_wav_file = '{}{}_{}{}_mix.wav'.format(speaker0, file_num0,
                                                  speaker1, file_num1)
        ...
        dataset.append((src, mix_spec, '{}-{}'.format(speaker0, speaker1)))
